...
/*!****************************!*\
  ...
  \****************************/
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var _handlers_js__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./handlers.js */ "./assets/js/handlers.js");
...
(function() {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const settingsForm = document.querySelector('.squeeze-settings-form');
        const wpThumbDefaults = ['thumbnail', 'medium', 'medium_large', 'large', 'full'];

        function getSqueezeWebpMode() {
            if (!settingsForm) {
                return 'direct';
            }
            const selected = settingsForm.querySelector('input[name="squeeze_options[webp_delivery_mode]"]:checked');
            return selected ? selected.value : 'direct';
        }

        function syncSqueezeWebpDependentUi() {
            if (!settingsForm) {
                return;
            }
            const mode = getSqueezeWebpMode();
            const cdnInput = settingsForm.querySelector('#squeeze_setting_cdn_url');
            const cdnTr = cdnInput ? cdnInput.closest('tr') : null;
            if (cdnTr) {
                cdnTr.style.display = '';
                cdnInput?.classList.remove('squeeze-hidden');
            }
            const webpFormatCheckbox = settingsForm.querySelector('input[name="squeeze_options[compress_formats][webp]"]');
            if (webpFormatCheckbox) {
                const wrap = webpFormatCheckbox.closest('.squeeze-suboption');
                const hint = wrap ? wrap.querySelector('.squeeze-hint') : null;
                if (mode === 'direct') {
                    webpFormatCheckbox.checked = true;
                    if (wrap) {
                        wrap.classList.add('squeeze-disabled');
                    }
                    if (hint) {
                        hint.style.display = 'block';
                    }
                } else if (wrap) {
                    wrap.classList.remove('squeeze-disabled');
                    if (hint) {
                        hint.style.display = 'none';
                    }
                }
            }
            settingsForm.querySelectorAll('.squeeze-webp-mode-card').forEach((card) => {
                const inp = card.querySelector('input.squeeze-webp-mode-input');
                card.classList.toggle('is-selected', !!(inp && inp.checked));
            });

            // Keep advanced section open when an advanced mode is selected.
            const advancedDetails = settingsForm.querySelector('.squeeze-webp-advanced');
            if (advancedDetails && (mode === 'sidecar' || mode === 'sidecar_replace')) {
                advancedDetails.open = true;
            }

            // Toggle WP Offload Media incompatibility notices per selected mode.
            const htaccessWarning = settingsForm.querySelector('.squeeze-offload-htaccess-warning');
            if (htaccessWarning) {
                const htaccessRadio = settingsForm.querySelector('input[name="squeeze_options[webp_delivery_mode]"][value="sidecar"]');
                htaccessWarning.classList.toggle('squeeze-notice--hidden', !(htaccessRadio && htaccessRadio.checked));
            }
            const sidecarWarning = settingsForm.querySelector('.squeeze-offload-sidecar-warning');
            if (sidecarWarning) {
                const sidecarReplaceRadio = settingsForm.querySelector('input[name="squeeze_options[webp_delivery_mode]"][value="sidecar_replace"]');
                sidecarWarning.classList.toggle('squeeze-notice--hidden', !(sidecarReplaceRadio && sidecarReplaceRadio.checked));
            }
        }

        settingsForm?.querySelectorAll('input[name="squeeze_options[webp_delivery_mode]"]').forEach((radio) => {
            radio.addEventListener('change', (event) => {
                syncSqueezeWebpDependentUi();
                if (!event.target.checked) {
                    return;
                }
                const initial = settingsForm.dataset.squeezeInitialWebpMode || getSqueezeWebpMode();
                if (event.target.value === initial) {
                    return;
                }
                window.alert(
                    __(
                        'Switching WebP delivery mode does not migrate existing files. Re-squeeze images after saving. Leftover JPEG/PNG + WebP pairs or orphaned files under squeeze-webp may remain until cleaned up.',
                        'squeeze'
                    )
                );
            });
        ...
        if (settingsForm) {
            settingsForm.dataset.squeezeInitialWebpMode = getSqueezeWebpMode();
        }
        syncSqueezeWebpDependentUi();
        ...
        document.getElementById('squeeze-thumbs-clear-all')?.addEventListener('click', () => {
            thumbGrid?.querySelectorAll('input[type="checkbox"]').forEach((cb) => {
                cb.checked = false;
            });
        });
        document.getElementById('squeeze-thumbs-wp-defaults')?.addEventListener('click', () => {
            if (!thumbGrid) {
                return;
            }
            thumbGrid.querySelectorAll('input[type="checkbox"]').forEach((cb) => {
                const id = cb.getAttribute('id') || '';
                const key = id.replace(/^squeeze_setting_compress_thumbs_/, '');
                cb.checked = wpThumbDefaults.includes(key);
            });
        });
    ...
    const MediaLibraryBulkSqueezeButton = wp.media.view.Button.extend({
        id: 'media-attachment-squeeze-button',
        className: 'media-button squeeze-bulk-button',
        text: __('Bulk Squeeze', 'squeeze'),
        initialize: function() {
            wp.media.view.Button.prototype.initialize.apply( this, arguments );
            this.controller.on( 'selection:toggle', this.toggleDisabled, this );
            this.controller.on( 'select:activate select:deactivate', this.toggleDisabled, this );
        },
        toggleDisabled: function() {
            this.model.set( 'disabled', ! this.controller.isModeActive( 'select' ) || ! this.controller.state().get( 'selection' ).length );
        ...
        handleThumbCheckbox(thumbCheckbox) {
            const thumbnail = thumbCheckbox.querySelector('.thumbnail');
            const centered = thumbnail.querySelector('.centered');
            ...
            thumbCheckbox.classList.add('uploading');
        },
        async onClick(event) {
            event.preventDefault();
            const btn = event.target;
            const deleteBtn = document.querySelector('.media-frame.mode-grid .delete-selected-button');
            const selected = this.controller.state().get('selection').toJSON();
            if (selected.length === 0) {
                alert(__('Please select at least one image.', 'squeeze'));
                return;
            }
            const uncompressedIDs = selected.map((attachment) => attachment.id);
        
            if (uncompressedIDs.length === 0) return;
        
            window.onbeforeunload = _handlers_js__WEBPACK_IMPORTED_MODULE_0__.handleOnLeave;
        
            uncompressedIDs.forEach((id) => {
                const thumbCheckbox = document.querySelector(`.media-frame.mode-grid .attachment[data-id="${id}"]`);
                this.handleThumbCheckbox(thumbCheckbox);
            });
            ...
            try {
                ...
                btn.textContent = __('Bulk Squeeze', 'squeeze');
	...
	wp.media.view.AttachmentsBrowser = wp.media.view.AttachmentsBrowser.extend({
		createToolbar() {
            ...
            this.toolbar.set(
                'MediaLibraryBulkSqueezeButton',
                new MediaLibraryBulkSqueezeButton({
                    controller: this.controller,
                    model: this.collection.props,
                    priority: -75,
                }).render()
