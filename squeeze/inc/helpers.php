...
class SqueezeHelpers extends SqueezeInit {
    ...
    public function upload_image_thumbs(
        $upload_path,
        $sizes,
        $file_format,
        $filename = '',
        ...
        $attach_meta = ( $attach_id > 0 ? wp_get_attachment_metadata( $attach_id ) : false );
        ...
        foreach ( $sizes as $size_name => $size_data ) {
            ...
            if ( $file_format === 'webp' && $is_direct_webp && $filename ) {
                ...
                // DO NOT appennd to the $filename, causes bug with complex filenames
            ...
            // Prefer the WordPress attachment metadata filename when available.
            $meta_size_key = ( $size_name === 'full' ? 'scaled' : $size_name );
            if ( is_array( $attach_meta ) && !empty( $attach_meta['sizes'][$meta_size_key]['file'] ) ) {
                $size_filename = basename( $attach_meta['sizes'][$meta_size_key]['file'] );
            }
            $size_filename = sanitize_file_name( $size_filename );
            $extension = strtolower( pathinfo( $size_filename, PATHINFO_EXTENSION ) );
            if ( $extension === 'jpeg' ) {
                $extension = 'jpg';
            }
            if ( $extension === '' || !in_array( $extension, $allowed_extensions, true ) ) {
                return new \WP_Error('squeeze_upload_image_thumbs_failed', '❌ ' . esc_html__( 'Invalid image format', 'squeeze' ));
            }
            if ( $size_name === 'full' ) {
                $size_name = 'scaled';
                unset($sizes['full']);
    ...
    public function upload_webp(
        ...
        $upload_file_webp = $this->upload_image(
            $upload_webp_path,
            $filename_webp,
            $decoded_webp,
            $is_file
    ...
    public function upload_webp_thumbs( $upload_path, $sizes_webp ) {
        ...
        foreach ( $sizes_webp as $size_name => $size_data ) {
            ...
            $size_filename = sanitize_file_name( basename( sanitize_url( $size_data['url'] ) ) );
            $extension = strtolower( pathinfo( $size_filename, PATHINFO_EXTENSION ) );
            if ( $extension === 'jpeg' ) {
                $extension = 'jpg';
            }
            if ( $extension === '' || !in_array( $extension, $allowed_extensions, true ) ) {
                return new \WP_Error('squeeze_upload_webp_thumbs_failed', '❌ ' . esc_html__( 'Invalid image format', 'squeeze' ));
            }
    ...
    public function get_direct_webp_filename( $dirname, $desired_filename, $exclude_attach_id = 0 ) {
        $desired_filename = sanitize_file_name( $desired_filename );
        $desired_path = trailingslashit( $dirname ) . $desired_filename;
        if ( !file_exists( $desired_path ) ) {
            return $desired_filename;
        }
        if ( !$this->is_uploads_path_owned_by_other_attachment( $desired_path, $exclude_attach_id ) ) {
            wp_delete_file( $desired_path );
            return $desired_filename;
        }
        return wp_unique_filename( $dirname, $desired_filename );
    }
     ...
     * @return bool
     */
    public function is_uploads_path_owned_by_other_attachment( $absolute_path, $exclude_attach_id = 0 ) {
    ...
    public function convert_image_path_to_webp_path( $image_path ) {
        ...
        $pattern = sprintf( '#%s[\\/\\\\]#', preg_quote( $content_folder, '#' ) );
    ...
    public function set_options( $options ) {
        ...
        if ( isset( $options['compress_formats'] ) && is_array( $options['compress_formats'] ) ) {
            $options['compress_formats'] = array_intersect_key( $options['compress_formats'], self::ALLOWED_IMAGE_FORMATS );
    ...
    /**
     * Opt-in cleanup of plugin metadata on uninstall.
     * Static so it can be used with register_uninstall_hook (free) and Freemius after_uninstall (premium).
     * Does not delete image files (originals, backups, or WebP copies).
     */
    public static function uninstall_cleanup() {
        $options = \get_option( 'squeeze_options', array() );
        if ( empty( $options['clear_data_on_uninstall'] ) ) {
            return;
        }
        ...
        global $wpdb;
        $wpdb->delete( $wpdb->postmeta, array(
            'meta_key' => 'squeeze_is_compressed',
        ) );
        $wpdb->delete( $wpdb->postmeta, array(
            'meta_key' => 'squeeze_compression_failed',
        ) );
    ...
    public function get_uncompressed_images( $last_id = 0 ) {
        ...
        while ( count( $results ) < self::$MEDIA_PER_PAGE && $max_iterations-- > 0 ) {
            ...
            $batch = ( empty( $excluded_images ) ? $posts : $this->filter_excluded_attachment_ids( $posts, $excluded_images ) );
    ...
    public function is_attachment_excluded( $attachment_id, $excluded_images = null ) {
        ...
        $candidates = array_filter( array(wp_get_attachment_url( $attachment_id ), wp_get_original_image_url( $attachment_id ), get_attached_file( $attachment_id )) );
    ...
    public function filter_excluded_attachment_ids( array $attachment_ids, $excluded_images = null ) {
        ...
        return array_values( array_filter( $attachment_ids, function ( $attachment_id ) use($excluded_images) {
    ...
    public function get_default_value( $option, $all = false ) {
        $options_defaults = apply_filters( 'squeeze_options_default', array(
            'jpeg_quality'                 => 80,
            'jpeg_baseline'                => false,
