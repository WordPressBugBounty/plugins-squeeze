...
class SqueezeHandlers extends SqueezeInit {
	...
	public function __construct() {
		...
		add_action('add_attachment', [$this, 'maybe_mark_voxel_client_compressed'], 10, 1);
		...
		add_action('shutdown', [$this, 'output_buffer_end'], 0); // test this

		add_filter('wp_prepare_attachment_for_js', [$this, 'update_attachment_metadata_for_js'], 10, 3);
		add_filter('wp_get_attachment_metadata', [$this, 'update_attachment_metadata'], 10, 2);
	}
	 ...
	 * @return void
	 */
	public function maybe_mark_voxel_client_compressed( $attach_id ) {
	...
	public function update_attachment() {
		...
		$sizes = isset($_POST["base64Sizes"]) ? (array) $_POST["base64Sizes"] : array(); // DO NOT SANITIZE because it's an array
		...
		$url = isset($_POST["url"]) ? sanitize_text_field($_POST["url"]) : ''; // sanitize_url() replaces spaces with %20, so we use sanitize_text_field() instead
		$process = isset($_POST["process"]) ? sanitize_text_field($_POST["process"]) : ''; // process: all, uncompressed, path
		...
		if ( $is_excluded = self::$SqueezeHelpers->is_excluded_image( $url, $excluded_images ) ) {
			wp_send_json_error('❌ '.esc_html__('Attachment is excluded from compression', 'squeeze') . ' ('. esc_html__('found substring: ', 'squeeze') . $is_excluded['exclude_reason'] . ')');
		}
		...
		if ($file_format === 'webp' && $extension !== 'webp' && $is_direct_webp && $process !== 'path') {
			...
			$file = get_attached_file($attach_id); // get the _wp_attached_file meta value which looks like "2023/10/image.jpg"
			$path_info = pathinfo($file);
			$dirname = $path_info['dirname'];
			//Get a unique filename in that folder
			$unique_filename = wp_unique_filename(
				$dirname,
				sanitize_file_name( preg_replace('/\.[^.]+$/', '.webp', $filename) ),
			);
			$webp_file_path = trailingslashit($dirname) . $unique_filename; // looks like "E:\Extra-Time\test/wp-content/uploads/2023/10/image.webp"

			$filename = basename($unique_filename);
			$url = str_replace(ABSPATH, home_url('/'), $webp_file_path); // convert file path to URL
		...
		if ($is_backup_original && $process !== 'path') { // do not backup for non library images
			// backup original
			if ($original_file) {
		...
		if ($process !== 'path') {
			...
			if ($file_format === 'webp' && $extension !== 'webp' && $is_direct_webp) {
				update_attached_file($attach_id, $webp_file_path); // update the _wp_attached_file meta value to the new webp file path
				wp_update_post([
					'ID' => $attach_id,
			...
			$sizes = self::$SqueezeHelpers->upload_image_thumbs($upload_path, $sizes, $file_format, $filename);
	...
	public function get_attachment() {
		...
		// Load excluded patterns once per request (cached in SqueezeHelpers::get_excluded_images()).
		$excluded_images = self::$SqueezeHelpers->get_excluded_images();

		$full_image = wp_get_attachment_image_src($attach_id, 'full');
		if ( ! empty( $excluded_images ) && ! empty( $full_image[0] ) ) {
			if ( $is_excluded = self::$SqueezeHelpers->is_excluded_image( $full_image[0], $excluded_images ) ) {
				wp_send_json_error('❌ '.esc_html__('Attachment is excluded from compression', 'squeeze') . ' ('. esc_html__('found substring: ', 'squeeze') . $is_excluded['exclude_reason'] . ')');
			}
		}
		...
		foreach ($pathes as $path) {
			...
			foreach ($images as $image) {
				...
				if ( ! empty( $excluded_images ) ) {
					if ( $is_excluded = self::$SqueezeHelpers->is_excluded_image( $attach_url, $excluded_images ) ) {
						$attachment_data[] = array(
							'excluded' => true,
							'exclude_reason' => $is_excluded['exclude_reason'],
							'filename' => $filename,
						);
						continue;
					}
	...
	/**
	 * AJAX: whether a filename matches the exclusion list (used before plupload compression).
	 */
	public function check_file_excluded() {
		check_ajax_referer( 'squeeze-nonce', '_ajax_nonce' );

		if ( ! isset( $_POST['fileName'] ) || $_POST['fileName'] === '' ) {
			wp_send_json_error( '❌ ' . esc_html__( 'File name not found', 'squeeze' ) );
		}

		$file_name       = sanitize_text_field( wp_unslash( $_POST['fileName'] ) );
		$excluded_images = self::$SqueezeHelpers->get_excluded_images();
		$result          = self::$SqueezeHelpers->is_excluded_image( $file_name, $excluded_images );
	...
	public function get_directories() {
		...
		if ( $parent_directory === '' ) {
			...
			// Parent from UI is site-relative (/wp-content/foo/); resolve under WP_CONTENT_DIR only.
			$rel = self::$SqueezeHelpers->bulk_parent_relative_to_content_dir( $parent_directory );
			if ( '' !== $rel && preg_match( '#(^|/)\.\.(/|$)#', $rel ) ) {
				wp_send_json_error( '❌ ' . esc_html__( 'Invalid directory path.', 'squeeze' ) );
			}
			$candidate = $allowed_base . ( '' !== $rel ? '/' . $rel : '' );
			$base_dir    = realpath( $candidate );
		...
		$output = array_map(function ($dir) use ($base_dir) {
			...
			if ($dir === $base_dir) {
				$path        = self::$SqueezeHelpers->bulk_directory_uri_from_filesystem( $dir . '/' );
				$parent_path = self::$SqueezeHelpers->bulk_directory_uri_from_filesystem( dirname( $base_dir ) . '/' );
	...
	public function output_buffer_start() {
		if ( ( !is_admin() 
			&& (!isset($_SERVER['HTTP_X_WP_REMOTE_REQUEST']) || $_SERVER['HTTP_X_WP_REMOTE_REQUEST'] !== 'true') )
	...
	public function output_buffer_end() {
		while (ob_get_level() > 0) {
			@ob_end_flush();  // Suppress notices; flushes modified content
		}
	}
	...
	public function replace_image_urls_with_webp($content) {
		...
		$callback = function ($matches) use ($is_direct_webp, $is_replace_urls) {
			...
			if ($file_extension === 'webp') {
				return $full_match; // Already a WebP image, no need to replace.
			}
			...
			if ( $is_direct_webp && ( $file_path === '' || ! file_exists( $file_path ) ) ) {
				...
				foreach ($webp_candidates as $candidate) {
					...
					$candidate_path = self::$SqueezeHelpers->resolve_media_url_to_abspath( $candidate_full );
			...
			return $full_match; // Fallback to the original URL if WEBP file doesn't exist.
		};
