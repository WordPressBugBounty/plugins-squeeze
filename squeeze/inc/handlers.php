...
class SqueezeHandlers extends SqueezeInit {
    public function __construct() {
        ...
        add_action(
            'add_attachment',
            [$this, 'maybe_mark_voxel_client_compressed'],
            10,
            1
        );
    ...
    public function update_attachment() {
        ...
        $file_format = strtolower( (string) $file_format );
        // JS sends MIME subtype "jpeg"; allowlist key is "jpg".
        if ( $file_format === 'jpeg' ) {
            $file_format = 'jpg';
        }
        ...
        $source_abspath = self::$SqueezeHelpers->resolve_media_url_to_abspath( $url );
        $did_direct_rename = false;
        // Direct WebP: write sibling .webp (library or path/bulk-from-page) and reclaim orphans.
        if ( $file_format === 'webp' && $extension !== 'webp' && $is_direct_webp ) {
            $desired_webp = sanitize_file_name( preg_replace( '/\\.[^.]+$/', '.webp', $filename ) );
            if ( $process !== 'path' && $attach_id > 0 ) {
                $file = get_attached_file( $attach_id );
                $dirname = ( $file ? dirname( $file ) : untrailingslashit( $upload_path ) );
            } else {
                $dirname = ( $source_abspath !== '' ? dirname( $source_abspath ) : untrailingslashit( $upload_path ) );
            }
            $unique_filename = self::$SqueezeHelpers->get_direct_webp_filename( $dirname, $desired_webp, $attach_id );
            ...
            $upload_path = trailingslashit( $dirname );
            $did_direct_rename = true;
        }
        if ( $original_file ) {
    ...
    public function replace_image_urls_with_webp( $content ) {
        ...
        $callback = function ( $matches ) use($is_direct_webp, $is_replace_urls) {
            ...
            if ( $is_replace_urls ) {
                ...
                // Check if the exact sized WEBP sidecar exists on the server (local disk check).
                ...
                // Fallback: sized/scaled/rotated URL → base (unsuffixed) .webp sidecar.
                $base_webp_candidates = array();
                if ( preg_match( '/-\\d+x\\d+\\.[^.]+\\.webp$/i', $webp_url ) ) {
                    $base_webp_candidates[] = preg_replace( '/-\\d+x\\d+(\\.[^.]+)\\.webp$/i', '$1.webp', $webp_url );
                }
                if ( preg_match( '/-scaled\\.[^.]+\\.webp$/i', $webp_url ) ) {
                    $base_webp_candidates[] = preg_replace( '/-scaled(\\.[^.]+)\\.webp$/i', '$1.webp', $webp_url );
                }
                if ( preg_match( '/-rotated\\.[^.]+\\.webp$/i', $webp_url ) ) {
                    $base_webp_candidates[] = preg_replace( '/-rotated(\\.[^.]+)\\.webp$/i', '$1.webp', $webp_url );
                }
                foreach ( $base_webp_candidates as $candidate_webp_url ) {
                    if ( $candidate_webp_url === $webp_url ) {
                        continue;
                    }
                    $candidate_path = self::$SqueezeHelpers->resolve_media_url_to_abspath( $protocol . $candidate_webp_url );
                    if ( $candidate_path !== '' && file_exists( $candidate_path ) ) {
                        return $candidate_webp_url;
                    }
                }
            }
            // Direct WebP: prefer a sibling .webp when present (e.g. hardcoded CSS bg still points at .jpg
            // after conversion, or path/bulk left both files on disk).
            if ( $is_direct_webp ) {
